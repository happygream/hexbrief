    ...
    <div>
      ...
      {!loading && items.map((item, i) => (
        <a key={i} href={item.link} target="_blank" rel="noopener noreferrer"
          ...
          <div className="nt" style={{ fontSize: 15, color: 'var(--paper)', fontWeight: 400, lineHeight: 1.45, transition: 'color .15s' }}>{item.title}</div>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'var(--muted)', letterSpacing: '0.08em', textTransform: 'uppercase', marginTop: 4 }}>{item.source}</div>
        </a>
      ))}
    </div>
  );
